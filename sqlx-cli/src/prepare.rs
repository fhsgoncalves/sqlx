...
fn load_query_manifest(path: &Path) -> anyhow::Result<QueryManifest> {
    match fs::read(path) {
        ...
        Err(err) => {
            Err(err).with_context(|| format!("failed to read query manifest: {}", path.display()))
        }
    }
}
...
fn scan_package_queries(package: &Package) -> anyhow::Result<PackageQueries> {
    ...
    for rust_file in rust_files_in(package.manifest_dir())? {
        ...
        let syntax = syn::parse_file(&source).with_context(|| {
            format!("failed to parse Rust source file: {}", rust_file.display())
        })?;
    ...
    while let Some(path) = dirs.pop() {
            ...
            .with_context(|| format!("failed to read directory: {}", path.display()))?
        {
            ...
            if file_type.is_dir() {
                ...
                && entry_path
                    .extension()
                    .is_some_and(|ext| ext == OsStr::new("rs"))
            {
...
fn extract_query_text(mac: &Macro, package_dir: &Path) -> anyhow::Result<Option<String>> {
    ...
    match ident.as_str() {
        "query" | "query_unchecked" | "query_scalar" | "query_scalar_unchecked" => {
            let args: QueryMacroArgs = syn::parse2(mac.tokens.clone())
                .with_context(|| format!("failed to parse macro arguments for `{ident}!`"))?;
            Ok(eval_string_expr(&args.query))
        }
        "query_as" | "query_as_unchecked" => {
            let args: QueryAsMacroArgs = syn::parse2(mac.tokens.clone())
                .with_context(|| format!("failed to parse macro arguments for `{ident}!`"))?;
            Ok(eval_string_expr(&args.query))
        }
        "query_file"
        ...
        | "query_file_scalar_unchecked" => {
            let args: QueryFileMacroArgs = syn::parse2(mac.tokens.clone())
                .with_context(|| format!("failed to parse macro arguments for `{ident}!`"))?;
            Ok(Some(read_query_file(package_dir, &args.path)?))
        }
        "query_file_as" | "query_file_as_unchecked" => {
            let args: QueryFileAsMacroArgs = syn::parse2(mac.tokens.clone())
                .with_context(|| format!("failed to parse macro arguments for `{ident}!`"))?;
            Ok(Some(read_query_file(package_dir, &args.path)?))
        }
        _ => Ok(None),
...
fn eval_string_expr(expr: &Expr) -> Option<String> {
    match expr {
        ...
        Expr::Binary(ExprBinary {
            ...
            "{}{}",
            eval_string_expr(left)?,
            eval_string_expr(right)?
        )),
